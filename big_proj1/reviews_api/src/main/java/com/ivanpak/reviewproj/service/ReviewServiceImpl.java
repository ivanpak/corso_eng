...
public class ReviewServiceImpl implements ReviewService {
    ...
    public ReviewDTO findByUuid(String uuid) {
        return modelToDTO(reviewRepository.findByUuid(uuid).orElseThrow(RuntimeException::new));
    }
    ...
    public Double getAvgScore(String bookUuid) {
        Double avg = reviewRepository.findByBookUuid(bookUuid)
                .stream()
                .map(r -> r.getStars())
                .mapToDouble(n -> (double)n)
                .average()
                .orElse(0D);
        // System.out.println("in getAvgScore: " + avg);
        return avg;
    }
